Static_Func = function(x,y,z,a,b,d,j, m, f) {
  ...
  withProgress(message = "Calculations in progress, please wait...", {
    ...
    B.NA = anyNA(Baseline_Dataset$reBAP)
    S.NA = anyNA(Stressor_Dataset$reBAP)
    
  if (B.NA == T) { 
    BaseMiss_Index <<- Baseline_Dataset$Time[is.na(Baseline_Dataset$reBAP)]
      B.Miss.interval <<-  which(diff(BaseMiss_Index) > 0.0015)
      B.Miss.interval.start <<-  BaseMiss_Index[c(1, B.Miss.interval + 1)]
      B.Miss.interval.end <<- BaseMiss_Index[c(B.Miss.interval, length(BaseMiss_Index))]
        B.Miss.DF <<- data.frame(Start = B.Miss.interval.start, 
                               End = B.Miss.interval.end)
        
  }
    
  if (S.NA == T) {
    StressMiss_Index <<- Stressor_Dataset$Time[is.na(Stressor_Dataset$reBAP)]
      s.Miss.interval <<-  which(diff(StressMiss_Index) > 0.0015)
      s.Miss.interval.start <<-  StressMiss_Index[c(1, s.Miss.interval + 1)]
      s.Miss.interval.end <<- StressMiss_Index[c(s.Miss.interval, length(StressMiss_Index))]
        S.Miss.DF <<- data.frame(Start = s.Miss.interval.start, 
                                End = s.Miss.interval.end)
    ...
    Base_BWFiltered_LMCAv = bw_filter(Baseline_Dataset$MedianFilteredMCAv, n = 8, W = BW_Cutoff, zero_lag = TRUE)
    Base_BWFiltered_MAP = bw_filter(Baseline_Dataset$SplineMAP, n = 8, W = BW_Cutoff, zero_lag = TRUE)
    
    Baseline_Dataset$FilteredLMCAv = Base_BWFiltered_LMCAv
    Baseline_Dataset$FilteredMAP = Base_BWFiltered_MAP
    
    
    Stress_BWFiltered_LMCAv = bw_filter(Stressor_Dataset$MedianFilteredMCAv, n = 8, W = BW_Cutoff, zero_lag = TRUE)
    Stress_BWFiltered_MAP = bw_filter(Stressor_Dataset$SplineMAP, n = 8, W = BW_Cutoff, zero_lag = TRUE)
    ...
    if (B.NA == T) {
    for (i in 1:length(B.Miss.DF$Start)) {
    Base_Matrix_Df$CrCP[Base_Matrix_Df$Time > floor(B.Miss.DF$Start[i]) & Base_Matrix_Df$Time < ceiling(B.Miss.DF$End[i])] = NA
    ...
    #Remove first and last detected beats (due to likely half cardiac cycles at start and end of stage)
    Base_end_beat = length(Base_Matrix_Df$Beat) 
    Base_Matrix_Df = Base_Matrix_Df[- Base_end_beat,]
    Base_Matrix_Df = Base_Matrix_Df[-1,]
  ...
  if (S.NA == T) {  
    ...
    #Remove first and last detected beats (due to likely half cardiac cycles at start and end of stage)
    Stress_end_beat = length(Stress_Matrix_Df$Beat) 
    Stress_Matrix_Df = Stress_Matrix_Df[- Stress_end_beat,]
    Stress_Matrix_Df = Stress_Matrix_Df[-1,]
    # -----------------------------------------------------------------------------#
